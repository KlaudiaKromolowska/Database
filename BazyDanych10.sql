...
select p.nazwa, p.opis, p.cena from pudelka p where idpudelka in (
select z.idpudelka from zawartosc z NATURAL JOIN czekoladki cz
...
select nazwa, opis, cena from pudelka where idpudelka in 
select distinct idpudelka from zawartosc where idczekoladki in
(select idczekoladki from czekoladki where nadzienie = 'truskawki'));
select nazwa, opis, cena from pudelka where exists (select idpudelka from czekoladki INNER JOIN zawartosc using (idczekoladki) WHERE id


select p.nazwa, p.opis, p.cena from pudelka p where idpudelka in (
select z.idpudelka from zawartosc z NATURAL JOIN czekoladki cz
WHERE nadzienie='trusk')
...
SELECT * FROM punkty_pomiarowe pp WHERE
50<ALL(select p.poziom_wody from pomiary p WHERE p.id_punktu=pp.id_punktu)

SELECT * FROM punkty_pomiarowe pp WHERE
10>ANY(SELECT o.zmiana_poziomu FROM ostrzezenia o WHERE o.id_punktu=pp.id_punktu)


SELECT g.nazwy FROM gmina WHERE NOT EXISTS
(select pp.id_punktu FROM punkty_pomiarowe pp
JOIN ostrzezenia o ON (id_punktu)
WHERE pp.id_gminy=g.identyfikator)

SELECT g.nazwy FROM gminy g WHERE EXISTS 
(select pp.id_punktu from punkty_pomiarowe LEFT JOIN pomiary p ON p.id_punktu=pp.id_punktu
WHERE p.id_pomiaru is NULL and pp.id_gminy=g.identyfikator)
...
Klienci, ktorzy zlozyli co najmniej 1 zamowienie (wersja z IN):

SELECT * FROM klienci WHERE idklienta IN (SELECT idklienta FROM zamowienia)

==================================================================================

Klienci, ktorzy nie zlozyli zadnych zamowien (wersja z IN):
...
select * from klienci where idkienta in (select id klienta from zamowienia)
select * from klienci where exists (select id klienta from zamowienia where idklienta=idklienta)
